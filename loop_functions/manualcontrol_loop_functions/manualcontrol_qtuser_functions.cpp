...
void CManualControlQTUserFunctions::EntitySelected(CEntity& c_entity) {
   ...
   CKheperaIVEntity* pcFB = dynamic_cast<CKheperaIVEntity*>(&c_entity);
   if(!pcFB) return;
   /* It's a foot-bot; extract its controller */
   m_pcController = dynamic_cast<CKheperaIVManualControl*>(&pcFB->GetControllableEntity().GetController());
